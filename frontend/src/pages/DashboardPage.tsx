...
export default function DashboardPage() {
  const { t } = useLocale();
  const dashboard = useAsync(() => dashboardApi.getMyDashboard(), []);
  ...
  if (data.total_scans === 0) {
    return (
      <EmptyState
        ...
        body={t.dashboard.noScansYetBody}
        action={
          <Link to="/scan">
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in">
        ...
        <Card>
          ...
          {data.most_common_diseases.length === 0 ? (
            ...
            <ul className="flex flex-col gap-3">
              {data.most_common_diseases.slice(0, 6).map((d) => (
                <li key={d.name} className="flex items-center gap-3">
                  ...
                  <span className="font-data w-6 shrink-0 text-end text-sm text-muted">{d.count}</span>
                </li>
              ))}
            </ul>
          )}
      ...
      <Card className="bg-primary-light/40">
        <CardHeader>
          <CardTitle>{t.dashboard.palmScans}</CardTitle>
        </CardHeader>
        <p className="font-data text-3xl font-semibold text-primary-dark">{data.palm_disease_stats.total_palm_scans}</p>
      </Card>
    </div>
  );
}
